 ...
 */
export const simplifiedSchemaToTypeDefinitionString = (simplifiedSchema) => {
    if (!simplifiedSchema)
        return "";
    ...
    if (simplifiedSchema.type === "array" &&
        ...
        const parts = simplifiedSchema.items
            .map((x) => simplifiedSchemaToTypeDefinitionString(x.schema))
            .join(" | ");
        const partsString = simplifiedSchema.items && simplifiedSchema.items.length >= 2
            ? `(${parts})[]`
            : `${parts}[]`;
        return partsString;
    }
    if (simplifiedSchema.type === "object" && simplifiedSchema.properties) {
        const objectParts = simplifiedSchema.properties.map((prop) => {
            ...
            const propertyString = `${descriptionString}${prop.name}${punctuationString}${simplifiedSchemaToTypeDefinitionString(prop.schema)};`;
